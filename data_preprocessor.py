...
import random
import settings
from utils import creat_adj,save_indexs
# Constants
min_seq_len = 3
min_seq_num = 3
...
def generate_sequence(input_data, min_seq_len, min_seq_num):
    ...
    seq_count = 0  # for statistics only用于统计生成的序列的数量。
    ...
    for user in input_data['UserId'].unique():  # process sequences for each user
        ...
        unique_date_group = user_visits.groupby([user_visits['Local_sg_time'].dt.date]) #根据用户访问天数分组
        for date in unique_date_group.groups:  # process sequences on each day
            ...
            if len(single_sequence) >= min_seq_len:  # filter sequences too short过滤掉某一用户某一天的轨迹长度过短的日轨迹
                user_sequences.append(single_sequence) #保存用户所有的访问轨迹编号list[[1,2,3],[2,3]..]
                user_sequences_meta.append((date, len(single_sequence)))#保存用户某一天的访问轨迹长度（日期，轨迹长度）
                seq_count += 1
...
def _reIndex_3d_list(input_list):
    ...
    def _flatten_3d_list(input_list):
        """flattern a 3d list to 1d 将三维数组转为一维数组
...
def generate_POI_sequences(input_data, visit_sequence_dict):#input_data=poi点信息，visit_sequence_dict每个用户对应的日访问轨迹
...
def generate_category_sequences(input_data, visit_sequence_dict):
    ...
    for user in visit_sequence_dict:
        ...
        for seq in visit_sequence_dict[user]:
            single_cat_sequence = []
            for visit in seq:
                single_cat_sequence.append(input_data['L1_Category'][visit]) #deng changed .used to be "L1_Category"
...
def generate_input_samples(feature_sequences, valid_input_index):
    ...
    return input_samples #这里已经不分用户了，输出所有用户的有效的长短期轨迹组，这里的轨迹组中包含的所有日轨迹都包含了该日轨迹中的poiId
...
def reshape_data(original_data):
    ...
    """
    result = []
    #将original_data转换为一个numpy的多维数组，并交换第一维和第二维的顺序，赋值给samples
...
def dump_data(data, city, data_type):
    ...
    """
    directory = f'./processed_data/{city}'
...
def generate_data(city):
    ...
    city=f"{city}_10cates"

    visit_sequence_dict, total_sequences_meta = generate_sequence(data, min_seq_len, min_seq_num)
    #visit_sequence_dict中存储每个用户的日轨迹信息，total_sequences_meta存储每个用户日轨迹对应的日期和长度
    valid_input_index = filter_long_short_term_sequences(total_sequences_meta, min_short_term_len, pre_seq_window,
                                                             min_long_term_count)#valid_input_index中存储这每一个用户它所有的日轨迹组中，哪几个日轨迹组可以组成长短期轨迹组
    '''
    SIN:Generated 31955 sequences in total for 2200 users
        Filtered 6320 valid input long+short term sequences for 800 users.
        SIN_10cates has 8974 pois
    '''
    train_data, valid_data, test_data, train_valid_data, meta_data = [], [], [], [], {}

    # POI inputs
    poi_sequences, poi_mapping = generate_POI_sequences(data, visit_sequence_dict)#poi_sequences中存储中的poiID经过index处理，存储的任然时168名用户对应的日轨迹
    poi_input_data = generate_input_samples(poi_sequences, valid_input_index)#输入所有有用的长短期轨迹组，其中每个长短期轨迹组中包含了每条轨迹的poiId信息，一共384组长轨迹组
    poi_train, poi_valid, poi_test, poi_train_valid = split_train_test(poi_input_data)
    ##creat poi adj
    pois_num = len(poi_mapping)
    print(f"{city} has {pois_num} pois")
    creat_adj(poi_train_valid, pois_num, city,"poi")
    save_indexs(poi_mapping,pois_num,city,"poi")#deng add
    ...
    ##creat poiCate adj
    poisCate_num = len(cat_mapping)
    print(f"{city} has {poisCate_num} poiCates")
    creat_adj(cat_train_valid, poisCate_num, city,"poiCate")  # deng add 构造邻接矩阵
    save_indexs(cat_mapping, poisCate_num, city, "poiCate")  # deng add 保存对应index
    ...
    ##creat poiCate adj
    users_num = len(user_mapping)
    print(f"{city} has {users_num} users")
    save_indexs(user_mapping, users_num, city, "user")  # deng add 保存对应index
    ...
    dump_data(train_data, city, "train_10cates")
    dump_data(valid_data, city, "valid_10cates")
    dump_data(test_data, city, "test_10cates")
    dump_data(train_valid_data, city, "train_valid_10cates")
    dump_data(meta_data, city, "meta_10cates")
